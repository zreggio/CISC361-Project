...
void waitQueue::insertNode(int jobNumber, int memNeeded, int maxDev, int runtime, int priority, int quantumTracker, int devHeld){
	Process* newProcess = new Process(jobNumber, memNeeded, maxDev, runtime, priority, quantumTracker, devHeld);
...
void waitQueue::deleteNode(int nodeOffset)
{
    Process *temp1 = head, *temp2 = NULL;
    int ListLen = 0;
...
void finished::printList(){
	...
	// Traverse the list.
	while (temp != NULL) {
		cout << "Job Number: " << temp->jobNumber << " ";
		if(temp->runtime == 0){
			cout << "Completed: Yes";
		}
		temp = temp->next;
...
void finished::insertNode(int jobNumber, int memNeeded, int maxDev, int runtime, int priority, int quantumTracker, int devHeld){
	Process* newProcess = new Process(jobNumber, memNeeded, maxDev, runtime, priority, quantumTracker, devHeld);
